...
echo "正则查找:"
# bat 过滤语义：未匹配不终止脚本
grep -E "^[0-9][0-9]*$" "numbers.txt" || true
...
# 使用 fc 和 comp
echo "文件比较:"
diff test1.txt test2.txt >/dev/null && echo "相同" || echo "不同"
cmp test1.txt test2.txt >/dev/null && echo "相同" || echo "不同"
...
# TODO: 手动检查: echo 脚本大小: %~z0

# 使用 %~$PATH:1 搜索路径
# TODO: 手动检查: echo PATH 中查找 notepad: %~$PATH:notepad

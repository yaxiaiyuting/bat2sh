...
| 本 session 动作 | ✅ **用户裁定修复**（C-1）：落地 `:1250`+`:1265`，6 条断言语义化，新增含点路径回归测试，跑满 D1–D4 后独立 commit（§7.1） |
...
⇒ **纯差分 oracle 对本缺陷是"默认盲区"**：L 侧沙箱根无点，若样本**自己不构造含点路径**，
`basename "${v%.*}"` 恰好给出正确值，**跑遍全部样本也测不出**。

> ⚠️ **本节的初版措辞过强（"结构性盲区"），已由 Phase B 的 oracle 实测更正。**
> `oracle-result-v2.md` §3.6 显示：**只要样本自己 `mkdir "my.project\samples"` 再 `cd` 进去**，
> L 侧路径就含点 ⇒ **oracle 能抓到**（样本 `w4` 正是如此，且修复后转为「一致」）。
> ⇒ 准确表述是：**默认盲区**（仪器路径布局所致），**不是不可测**；
> 要检出必须**主动构造含点路径**（或改采集器 run_dir 命名）。
...
> 主 session 先按暂停条件**自主裁定执行 C-0**（不落地）。
> **随后用户明确指示：「C 需要修复，可以开 subagent 来修复」** ⇒ **用户否决 C-0，改判 C-1。**
>
> 本 session 最终执行 **C-1**：
>
> - ✅ **落地** `_modifier:1250`（`%%~n`）+ `:1265`（`%~n0`）补丁；
> - ✅ 把 D2 实测的 **6 条实现串断言**改写为**真跑语义断言**（§5.2）；
> - ✅ **新增**覆盖「含点祖先路径」的回归测试（无此测试则本缺陷结构性抓不到，等于没修）；
> - ✅ 跑满 **D1–D4** 四道门后才 commit；
> - ✅ 结果记入 **`f-n-fix-result.md`**，**独立 commit**（与 Phase A/B 分开）；
> - ❌ `%%~x`（`:1252`）与 `%%~dpn`（`:1244`）**仍不修**（缺 cmd 真机指纹，见 §3.3）。
>
> **披露**：本次由 subagent 执行实现，主 session 按纪律 9 **逐项核验**（门结果 + 断言语义 + 改动面）。
> 核验结论记入 `f-n-fix-result.md` 的「主 session 核验」节。

...
# v1.4.1 沙箱复检：结论与 v1.5 方向判断（只读 + 归档）

> 诊断对象：HEAD = `d59465a`（v1.4.1 tag 指向 `1f5df95`；其后为 PKGBUILD sha256 同步与 release note 两个 commit）
> 方法：`tools/corpus-analysis/analyze.py`——114 文件（92 bat + 22 ps1）逐个转换 → 静态分析 →
> `bwrap --unshare-all` 沙箱运行（无网络、不挂载真实 HOME/etc/var/root；双 pass：strict / 关闭 strict 复跑）→ 关联分类。
> 危险构造（`mount` 类）跳过不运行；CC0 语料（665 ps1）本轮不重复分析。
> 本报告所有构造以归一化形式描述；无许可语料原文未以任何形式进入本文件。
> 完整数据（脱敏）：`tools/corpus-analysis/results-anonymized.json`。

## 10.1 总表（v1.4.0 基线 → v1.4.1 复检）

| 指标 | v1.4.0 基线 | v1.4.1 复检 |
|---|---|---|
| 文件数 | 114 | 114 |
| 转换 OK（无 TODO / 含 TODO） | — | 95（34 / 61） |
| 转换 DEGRADED / FAILED | 19 / 0 | 19 / 0 |
| **沙箱运行完全可用** | **48** | **54（+6）** |
| 转换通过但运行失败 | 45 | **39** |
| 运行失败分类 | MISSING_CMD 14 / MISSING_FILE 12 / RC_NONZERO 19 | MISSING_CMD 8 / MISSING_FILE 10 / RC_NONZERO 21 |
| **TIMEOUT / CRASH** | 0 / 0 | **0 / 0** |
| 关闭 `set -e` 后恢复（非缺陷） | 10 | 12 |

v1.4.1 修复 4 组（rem 标点变体 / goto:eof 无空格与链式 goto / call:label 无空格含静默丢行 /
`cd.`·`pause…`·`dir /a-d` / `set /p` 空变量名 / `robocopy /MIR` 开关与目录内容语义），
6 个脚本由失败变为完全可用（call-1、产生任意随机数、修改版批处理发邮件、更新音乐播放列表test、
根据TITLE重命名文件、生成随机字符串）。

## 10.2 交叉表：转换通过 95 × 运行结果

| 转换通过 × 运行 | 数量 | 判定 |
|---|---|---|
| OK（真正可用） | 54 | — |
| RC_NONZERO | 21 | 需逐例判断（其中 12 例为 `set -e` 早退、非缺陷） |
| MISSING_FILE | 10 | fixture 不全 / Windows 绝对路径 |
| MISSING_CMD | 8 | 环境缺失或转换残渣 |
| UNSAFE 跳过 | 2 | `mount` 类危险构造（hiddenDIsk.bat / mountvol.bat） |
| **CRASH** | **0** | **无崩溃类缺陷** |

## 10.3 深层失败归因（"关闭 `set -e` 后仍失败" 27 例）

| 类别 | 数量 | 明细（归一化） |
|---|---|---|
| 环境 / 沙箱 / fixture / 脚本自身 | 17 | 网络被沙箱阻断 3、ping 原始套接字权限 1、外部工具缺失 4（adb×2 / powercfg / cscript）、Windows 绝对路径 5、输入 fixture 缺失 2、脚本自身逻辑 2（除零 / 日期解析） |
| bat 转换器缺陷候选 | 6 | `set/a` 无空格未识别；for /f 命令串外层双引号未剥离；`%~0` 引号双写；`dir /b` 通配生成 `*.])` 可疑；管道目标未转换；`ren` 目标名异常 |
| PS 转换器缺陷候选 | 4 | `finally`/`process` 块、`[version]` 类型字面量、hashtable 键被当作命令执行 |

另有 19 例**整体降级**（保护性行为，不产出坏 bash）：bat 2（标签行含括号/特殊注释行）、
PS 17（`}` 与 `else` 之间夹注释导致 else 失联、`elseif` 条件 TODO 后悬空 `elif`、
`-split`/`-join`/`.Members()`/`Add-Type` 等表达式与对象模型残留）。

## 10.4 PS 侧单独统计（与 bat 侧对比）

| 侧 | 文件 | 转换 OK | DEGRADED | 运行 OK | 完全可用 |
|---|---|---|---|---|---|
| bat | 92 | 90 | 2 | 54 | **54（58.7%）** |
| ps1 | 22 | 5 | **17** | 0 | **0（0%）** |
...
1. **CRASH = 0、TIMEOUT = 0**：无崩溃类缺陷；"never emit broken bash" 降级守卫把 19 个结构性
   问题挡在运行之前（安全网有效）。
2. **不是"全部 MISSING_CMD"**：失败分布为 RC_NONZERO 21 / MISSING_FILE 10 / MISSING_CMD 8；
   27 例深层失败中 17 例属环境/沙箱/fixture/脚本自身，**10 例含转换器因素**（bat 6、PS 4）。
3. **PS 侧是最大缺口**：22 个 ps1 仅 5 个转换 OK、17 个整体降级、**0 个可运行**。
4. **bat 侧仍有具体可修的小缺陷**（6 例运行残渣 + 2 例降级根因），均非崩溃类，单点修复
   风险低、收益小。

按任务判据映射（CRASH>0 → 有真 bug；全 MISSING_CMD → 转换器到顶）：

- CRASH=0 但存在**非崩溃类真实缺陷**（bat ≈8 例、PS ≈21 例），**不满足"转换器到顶"条件**；
- 收益排序：**PS 结构/表达式收敛（17 降级 + 4 运行残渣）> bat 剩余小品（8 例）**；
- v1.5 可选方向（供人工决策，本报告不启动实现）：
  - **A（推荐）**：PowerShell 块结构（if/else 关联、elif 悬挂）与表达式运算符收敛——
    覆盖 17 个降级文件，是"可用率"最大单点；
  - B：bat 剩余小品（`set/a` 无空格、for /f 命令串引号、`%~0` 引号、glob 生成、标签行防御）；
  - C：非转换器方向（API 修复 / 文档 / 发行工程），与本次数据无冲突。

（报告完，含 §9、§10 与 v1.4.1 复检结论）

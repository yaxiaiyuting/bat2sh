...
GOTO_PATTERNS: tuple[ControlFlowPattern, ...] = (
    ...
    ControlFlowPattern(
        ...
        integrated=False,
        convertible=False,
        plan="v2.0 CFG：确定 goto→标签之间的直筒区间，改写为 if/else 或提前 return",
        evidence="获取某路径下的所有文件名.cmd:21",
        notes="需标签位置 + 可达性；目标可被多处跳入时不能仅当 skip。",
    ),
    ControlFlowPattern(
        key="backward_loop",
    ...
    ControlFlowPattern(
        ...
        integrated=False,
        convertible=False,
        plan="可安全降为 no-op；但命中文件均为语法失败/大文件，翻转价值 0 → 不做",
        evidence="合并文本/合并文本.bat:6",
        notes="v1.10.0b1 实测无余量（不落在任何 degraded 单设施文件上）。",
    ),
    ControlFlowPattern(
        key="missing_label",

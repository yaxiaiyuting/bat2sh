...
GOTO_PATTERNS: tuple[ControlFlowPattern, ...] = (
    ...
    ControlFlowPattern(
        ...
        integrated=False,
        convertible=False,
        plan="v2.4.0 保守子集（不可达区间注释化）+ v2.5.0 CFG 标签分派状态机（core/cfg_state.py）可覆盖一般前跳；门控默认关闭，默认路径维持诚实 TODO",
        evidence="获取某路径下的所有文件名.cmd:21",
        notes="可达性证明见 docs/v2.4.0-review.md；v2.5.0 状态机见 docs/v2.5.0-blockstack-design.md §5.2.2。**不 integrated**：门控（settings.cfg_state_machine）默认关闭，激活待逐文件 oracle（静默错底线）。",
    ),
    ControlFlowPattern(
        key="backward_loop",
        ...
        integrated=False,
        convertible=False,
        plan="v2.5.0 CFG 标签分派状态机（core/cfg_state.py）：回跳 → `pc=<臂>; continue`；门控默认关闭，默认路径维持诚实 TODO",
        evidence="带密码的批处理.bat:14",
        notes="出口不再需静态全知：按 pc 转移，出口由源内 goto/exit 表达（docs/v2.5.0-blockstack-design.md §5.2.2）。**不 integrated**：门控默认关闭，激活待逐文件 oracle（静默错底线）。",
    ),
    ControlFlowPattern(
        key="in_block_goto",
        ...
        integrated=False,
        convertible=False,
        plan="v2.5.0 CFG 标签分派状态机（core/cfg_state.py）：块内 goto → `continue N`（N = 包裹循环数 + 1）跳出到分派循环；门控默认关闭",
        evidence="xp下确定最后的盘符.bat:7",
        notes="v2.5.0 门控版已覆盖 `if`/`for` 体内 goto（含两层嵌套 → `continue 3`），由 tests/test_cfg_state.py 语义锁定；**不 integrated**：门控默认关闭，默认路径维持诚实 TODO。",
    ),
    ControlFlowPattern(
        key="redundant_goto",

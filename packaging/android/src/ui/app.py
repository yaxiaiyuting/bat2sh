...
布局（任务书 4.5，移动端用上下两栏而不是三栏）：
    工具栏  [选择文件] [转换] [保存] [校验] [运行]
    源代码  可编辑
    产物    只读 + 近似高亮
    报告/运行输出
"""
...
from ui import highlighter  # noqa: E402
...
class App:
    """极简可用版：不批量、不 API 修复、无设置对话框、无差异视图。"""
    ...
    def build(self) -> None:
        ...
        root = ft.Column(
            [
                ...
                ft.Text("Windows 批处理 / PowerShell -> Bash（Android 最小可用版）",
